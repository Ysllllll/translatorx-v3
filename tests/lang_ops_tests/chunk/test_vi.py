...
class TestVietnameseSplitter(SplitterTestBase):
    ...
    def test_split_by_length(self) -> None:
        # Character split
        # Multi-word split
        assert _ops.split_by_length("Xin chào bạn khỏe không", max_length=12) == ["Xin chào bạn", "khỏe không"]

        # Fit / empty / edge
        assert _ops.split_by_length("Xin chào", max_length=20) == ["Xin chào"]
        assert _ops.split_by_length("", max_length=10) == []

        # Errors
        import pytest
        with pytest.raises(ValueError):
            _ops.split_by_length("Xin chào", max_length=0)
        with pytest.raises(ValueError):
            _ops.split_by_length("Xin chào", max_length=-1)
        with pytest.raises(TypeError):
            _ops.split_by_length("Xin chào", max_length=5, unit="sentence")
