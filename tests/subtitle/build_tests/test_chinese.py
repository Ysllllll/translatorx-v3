...
def _extreme_length_segment() -> list[Segment]:
    ...
    return [
        S(
            "这是一个超级长的英文单词supercalifragilisticexpialidocious测试",
            0.0,
            5.0,
            words=[
                W("这", 0.0, 0.2),
...
class TestChineseMerge:
    def test_merge_clauses_back(self) -> None:
        ...
        clause_result = Subtitle(_asr_news_segments(), _ops).sentences().clauses().build()
        merged_result = Subtitle(_asr_news_segments(), _ops).sentences().clauses().merge(15).build()
        for seg in merged_result:
            assert _ops.length(seg.text) <= 15, f"Segment too long: {seg.text!r} ({_ops.length(seg.text)})"
        assert len(merged_result) <= len(clause_result)

    def test_merge_preserves_text(self) -> None:
        result = Subtitle(_asr_news_segments(), _ops).sentences().clauses().merge(20).build()
    ...
    def test_merge_nothing_fits(self) -> None:
        result = Subtitle(_short_segments(), _ops).sentences().merge(3).build()
        assert len(result) == 2
    ...
    def test_merge_chain_full(self) -> None:
        ...
        result = Subtitle(_asr_news_segments(), _ops).sentences().clauses().split(8).merge(15).build()
        for seg in result:
    ...
    def test_merge_respects_sentence_boundaries(self) -> None:
        ...
        sentence_count = len(Subtitle(_asr_news_segments(), _ops).sentences().build())
    ...
    def test_same_speaker_no_extra_splits(self) -> None:
        segments = [
            S(
                ...
                words=[
                    W("你", 0.0, 0.3, speaker="A"),
                    W("好", 0.3, 0.6, speaker="A"),
                    W("世", 0.6, 0.9, speaker="A"),
                    W("界", 0.9, 1.5, speaker="A"),
                    W("。", 1.5, 2.0, speaker="A"),
                    W("今", 2.0, 2.3, speaker="A"),
                    W("天", 2.3, 2.6, speaker="A"),
                    W("不", 2.6, 3.0, speaker="A"),
                    W("错", 3.0, 4.0, speaker="A"),
                    W("！", 4.0, 5.0, speaker="A"),
                ],
            )
        ]
