"""Shared base class for per-language SegmentBuilder tests.
...
class BuilderTestBase:
    ...
    def test_empty_input(self) -> None:
        """Builder on empty segment list produces empty result."""
        assert SegmentBuilder([], self.ops()).build() == []
        assert SegmentBuilder([], self.ops()).sentences().build() == []
        assert SegmentBuilder([], self.ops()).records() == []

    def test_immutability(self) -> None:
        """Chaining creates new builders; originals are unchanged."""
        seg = S("Hello.", 0.0, 1.0, words=[W("Hello.", 0.0, 1.0)])
        builder = SegmentBuilder([seg], self.ops())
    ...
    def test_build_preserves_all_text(self) -> None:
        ...
        seg = S("Hello.", 0.0, 1.0, words=[W("Hello.", 0.0, 1.0)])
        result = SegmentBuilder([seg], self.ops()).build()
    ...
    def test_build_segments_have_words(self) -> None:
        ...
        seg = S("Hello.", 0.0, 1.0, words=[W("Hello.", 0.0, 1.0)])
        result = SegmentBuilder([seg], self.ops()).sentences().build()
        for s in result:
    ...
    def test_records_structure(self) -> None:
        ...
        seg = S("Hello.", 0.0, 1.0, words=[W("Hello.", 0.0, 1.0)])
        records = SegmentBuilder([seg], self.ops()).records()
        for rec in records:
