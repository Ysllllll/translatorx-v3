...
def test_default_registry_no_app_registers_build_and_merge() -> None:
    ...
    assert not reg.is_registered("summary")
...
def test_default_registry_summary_builds_lazy_stage() -> None:
    class _App:
        def punc_restorer(self, lang):
            return None
    ...
    reg = make_default_registry(app=_App())  # type: ignore[arg-type]
    stage = reg.build(StageDef(name="summary", params={"window_words": 1000}))
