...
class TestSegmentDisplay:
    def test_segment_repr_shows_summary(self) -> None:
        segment = Segment(start=0.0, end=2.0, text="你好世界。", words=[Word("你", 0.0, 0.5), Word("好", 0.5, 1.0)])
        assert repr(segment) == "Segment(0.00->2.00, text='你好世界。', words=2)"

    def test_segment_pretty_shows_words(self) -> None:
        segment = Segment(start=0.0, end=2.0, text="Hello world.", speaker="A", words=[Word("Hello", 0.0, 1.0), Word("world.", 1.0, 2.0)], extra={"source": "asr"})
        assert segment.pretty() == ("Segment(\n  start=0.00,\n  end=2.00,\n  text='Hello world.',\n  speaker='A',\n  words=[\"Word('Hello', 0.00->1.00)\", \"Word('world.', 1.00->2.00)\"],\n  extra={'source': 'asr'},\n)")


class TestSentenceRecordDisplay:
    def test_sentence_record_repr_shows_summary(self) -> None:
        record = SentenceRecord(src_text="你好世界。", start=0.0, end=2.0, segments=[Segment(0.0, 2.0, "你好世界。")])
