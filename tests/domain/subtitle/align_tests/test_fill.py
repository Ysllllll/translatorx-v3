...
class TestFillWords:
    ...
    def test_whitespace_split(self):
        ...
        assert _word_view(result.words) == [{"word": "Hello", "start": pytest.approx(0.0), "end": pytest.approx(2.0)}, {"word": "world", "start": pytest.approx(2.0), "end": pytest.approx(4.0)}]

    def test_unequal_tokens(self):
        result = fill_words(Segment(start=0.0, end=10.0, text="I am fine"))
        assert _word_view(result.words) == [{"word": "I", "start": pytest.approx(0.0), "end": pytest.approx(10.0 * 1 / 7)}, {"word": "am", "start": pytest.approx(10.0 * 1 / 7), "end": pytest.approx(10.0 * 3 / 7)}, {"word": "fine", "start": pytest.approx(10.0 * 3 / 7), "end": pytest.approx(10.0)}]

    def test_custom_split_fn(self):
        result = fill_words(Segment(start=0.0, end=2.0, text="你好世界"), split_fn=list)
    ...
    def test_attached_punctuation(self):
        ...
        assert _word_view(result.words) == [{"word": "Hello,", "start": pytest.approx(0.0), "end": pytest.approx(3.0)}, {"word": "world!", "start": pytest.approx(3.0), "end": pytest.approx(6.0)}]

    def test_custom_split_fn_attaches_punctuation(self):
        result = fill_words(Segment(start=0.0, end=6.0, text="你好，世界！"), split_fn=list)
        assert _word_view(result.words) == [
