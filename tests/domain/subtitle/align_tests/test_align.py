...
def _seg_view(segs):
    return [{"text": s.text, "start": s.start, "end": s.end, "words": [w.word for w in s.words]} for s in segs]
...
class TestAlignSegments:
    def test_basic_alignment(self):
        ...
        assert _seg_view(segs) == [{"text": "Hello world.", "start": pytest.approx(0.0), "end": pytest.approx(1.0), "words": ["Hello", "world"]}, {"text": "How are you?", "start": pytest.approx(1.1), "end": pytest.approx(2.0), "words": ["How", "are", "you"]}]

    def test_single_chunk(self):
        words = [Word("Hi", 0, 0.5), Word("there", 0.6, 1)]
        segs = align_segments(["Hi there"], words)
        assert _seg_view(segs) == [{"text": "Hi there", "start": pytest.approx(0.0), "end": pytest.approx(1.0), "words": ["Hi", "there"]}]
    ...
    def test_empty_words(self):
        ...
        assert _seg_view(segs) == [{"text": "Hello world", "start": 0.0, "end": 0.0, "words": []}]
