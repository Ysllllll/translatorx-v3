...
class TestJapaneseSplitter(SplitterTestBase):
    ...
    def test_split_by_length(self) -> None:
        ...
        # チャンクチェーン
        assert _ops.chunk("これはテストです。次の文です。").sentences().split(8).result() == ["これはテスト", "です。", "次の文です。"]
        assert _ops.chunk("最初、次、最後。").clauses().split(4).result() == ["最初、", "次、", "最後。"]
    ...
    def test_split_long_text(self) -> None:
        ...
        assert _ops.split_sentences(self.TEXT_SAMPLE) == [
            ...
            "ある書道家は次のように語った。",
            "「技術は人間の感性を拡張するものである！」",
            "この言葉は、多くのクリエイターの共感を呼んだ……伝統工芸の分野でも、3D プリンターを活用した新しい技法が開発されつつある。",
