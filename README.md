...
Hexagonal（端口与适配器）分层，依赖严格由外向内（`api → application → adapters → ports → domain`）：

```
domain/       纯领域：Word / Segment / SentenceRecord / LangOps / Subtitle
ports/        抽象协议：Source / Processor / LLMEngine / MediaSource / ApplyFn / ErrorReporter
adapters/     具体实现：sources / storage / engines / parsers / media / preprocess / reporters
application/  用例编排：orchestrator / translate / checker / processors / config / observability
api/          用户入口：app（Builder 链）、trx（slim facade + 工厂函数）
```

由 `tests/test_architecture.py` 自动守护分层规则 —— 新文件会被自动纳入检测。

完整说明: [`CLAUDE.md`](CLAUDE.md)。
